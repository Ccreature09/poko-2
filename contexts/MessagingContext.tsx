...
export const MessagingProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    switch (user.role) {
      ...
      case "parent":
        newPermissions.canSendToTeachers = true;
        newPermissions.canSendToAdmins = true;
        newPermissions.canSendToStudents = true; // Allow parents to message students (their own children)
        break;
    }
  ...
  useEffect(() => {
    ...
    const usersCollection = collection(schoolRef, "users");
  ...
  const setCurrentConversation = useCallback(
    async (conversationId: string | null) => {
      if (!conversationId || !user || !user.userId || !user.schoolId) {
      ...
      try {
        ...
        const usersCollection = collection(schoolRef, "users");
        const userRef = doc(usersCollection, user.userId);
        const conversationsCollection = collection(userRef, "conversations");
        ...
        if (conversationSnap.exists()) {
          ...
          if (conversationData.unreadCount) {
            ...
            const hasUnread =
              typeof conversationData.unreadCount === "object"
                ? (conversationData.unreadCount[user.userId] || 0) > 0
                : typeof conversationData.unreadCount === "number" &&
                  conversationData.unreadCount > 0;

            if (hasUnread) {
              // Since we've already checked user.userId is not undefined above
    ...
    async (
      ...
      try {
        ...
        const conversationRef = doc(
          collection(schoolRef, "conversations"),
          conversationId
        );
        const conversationSnap = await getDoc(conversationRef);
        ...
        const newMessage: Message = {
          messageId: uuidv4(),
          senderId: user.userId || "", // Add fallback to empty string if userId is undefined
          content: content.trim(),
          timestamp: new Date().toISOString(),
          readBy: [user.userId || ""], // Ensure readBy also has non-undefined values
          status: "sent",
        ...
        for (const participantId of conversationData.participants) {
          ...
          const userConvRef = doc(
            collection(participantRef, "conversations"),
            conversationId
          );
          ...
          const userConvData = userConvSnap.exists()
            ...
            : {
                ...
                ...(conversationData.isGroup && conversationData.groupName
                  ? {
          ...
          const currentUnreadCount =
            typeof userConvData.unreadCount === "object"
              ? userConvData.unreadCount
              : { [user.userId || ""]: 0 }; // Ensure userId is never undefined when used as an index
  ...
  const createConversation = useCallback(
    async (
      ...
      if (!user || !user.userId || !user.schoolId) return null;
      ...
      try {
        ...
        for (const participantId of participants) {
          const usersCollection = collection(schoolRef, "users");
          const participantRef = doc(usersCollection, participantId);
  ...
  const markAsRead = useCallback(
    async (conversationId: string, messageIds: string[]): Promise<void> => {
      if (!user || !user.userId || !user.schoolId || messageIds.length === 0)
        return;
      ...
      try {
        ...
        const conversationData = conversationSnap.data() as Conversation;
        const userId = user.userId; // Store userId in a local variable since we've already checked it's not undefined
        ...
        const schoolConversationsCollection = collection(
          schoolRef,
          "conversations"
        );
        const masterConversationRef = doc(
          schoolConversationsCollection,
          conversationId
        );
  ...
  const searchMessages = useCallback(
    async (filter: MessageFilter): Promise<Conversation[]> => {
      if (!user) return [];
  ...
  const canMessageUser = useCallback(
    async (userId: string): Promise<boolean> => {
      if (!user || !user.userId || !user.schoolId) return false;
      ...
      try {
        ...
        switch (user.role) {
          ...
          case "student":
            ...
            return false;
          case "parent":
            // Parents can message teachers, admins, and their own children
            const parentSchoolRef = doc(db, "schools", user.schoolId);
            const usersCollection2 = collection(parentSchoolRef, "users");
            const targetUserRef = doc(usersCollection2, userId);
            ...
            if (targetUserSnap.exists()) {
              ...
              // Check if the target user is one of the parent's children
              if (targetRole === "student") {
                // Get the parent's data to access childrenIds
                const parentRef = doc(usersCollection2, user.userId);
                ...
                if (parentSnap.exists()) {
                  ...
                  // Allow messaging if the student is in the parent's childrenIds array
                  return childrenIds.includes(userId);
                }
              }
            }
            return false;
  ...
  const deleteMessage = useCallback(
    async (conversationId: string, messageId: string): Promise<boolean> => {
      ...
      try {
        ...
        const conversationData = conversationSnap.data() as Conversation;
        const message = conversationData.messages.find(
  ...
  const sendAnnouncement = useCallback(
    async (content: string, recipientRoles: string[]): Promise<boolean> => {
      ...
      try {
        ...
        // Flatten array and filter out any undefined/empty values
        const recipients = recipientsByRole.flat().filter((id) => id);
  ...
  const sendClassMessage = useCallback(
    async (classId: string, content: string): Promise<boolean> => {
      if (
        !user ||
      ...
      try {
        ...
        if (existingConversations.empty) {
          ...
          if (newConversationId) {
            classConversationId = newConversationId;
          }
        } else {
