...
import type { Subject, TeacherSubjectAssignment, ClassSubjectsMapping, HomeroomClass } from "@/lib/interfaces";
...
export const getClasses = async (schoolId: string): Promise<HomeroomClass[]> => {
...
export const getClassesByGradeLevel = async (schoolId: string, yearGroup: number): Promise<HomeroomClass[]> => {
  const classesCollection = collection(db, "schools", schoolId, "classes");
...
export const createTeacherSubjectAssignment = async (
  ...
  const assignmentsCollection = collection(db, "schools", schoolId, "teacherSubjectAssignments");
...
export const updateTeacherSubjectAssignment = async (
  ...
  const assignmentRef = doc(db, "schools", schoolId, "teacherSubjectAssignments", assignmentId);
...
export const getClassAssignments = async (
  ...
  const assignmentsCollection = collection(db, "schools", schoolId, "teacherSubjectAssignments");
  const q = query(assignmentsCollection, where("classIds", "array-contains", classId));
  const snapshot = await getDocs(q);
  return snapshot.docs.map(
    (doc) => ({ ...doc.data() } as TeacherSubjectAssignment)
  );
...
export const updateClassSubjects = async (
  ...
  for (const { subjectId, teacherId } of subjects) {
    ...
    if (snapshot.empty) {
      ...
      const existingData = existingAssignment.data() as TeacherSubjectAssignment;
