...
export async function getTeacherClasses(
  ...
  console.log(`Fetching classes for teacher ${teacherId} in school ${schoolId}`);
  ...
  const [classesSnapshot, homeroomSnapshot, subjectsSnapshot] = await Promise.all([
    getDocs(classesQuery),
    getDocs(homeroomQuery),
    getDocs(subjectsQuery)
  ]);
  
  console.log(`Classes where teacher is in teacherIds: ${classesSnapshot.size}`);
  console.log(`Classes where teacher is homeroom teacher: ${homeroomSnapshot.size}`);
  ...
  console.log(`Found ${classes.length} classes and ${subjects.length} subjects for teacher`);
...
export async function getClassesTaughtByTeacher(
  ...
  console.log(`Fetching timetable entries for teacher ${teacherId} in school ${schoolId}`);
  const schoolRef = doc(db, 'schools', schoolId);
  ...
  const processPromises = timetablesSnapshot.docs.map(async (timetableDoc) => {
    ...
    const teacherEntries = timetableData.entries.filter(entry => entry.teacherId === teacherId);
    ...
    for (const entry of teacherEntries) {
      ...
      if (!subjectData) {
        console.log(`Subject data not found for subject ID: ${entry.subjectId}`);
        continue;
      }
  ...
  console.log(`Found ${classesTaught.length} class sessions taught by teacher ${teacherId}`);
...
export async function doesClassSessionExist(
  ...
  console.log(`Checking if class session exists: class=${classId}, subject=${subjectId}, day=${dayOfWeek}, period=${periodNumber}`);
  ...
  for (const timetableDoc of timetablesSnapshot.docs) {
    ...
    if (matchingEntry) {
      console.log(`Found matching class session: ${JSON.stringify(matchingEntry)}`);
      return true;
    }
  }
