 ...
 */
export async function getUserByEmail(schoolId: string, email: string): Promise<UserData | null> {
  try {
 ...
 */
export function formatRelativeTime(timestamp: Timestamp | Date | string | number): string {
  if (!timestamp) return '';
