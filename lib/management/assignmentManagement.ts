...
export const submitAssignment = async (
  ...
  submission: Omit<
    AssignmentSubmission,
    "submissionId" | "submittedAt" | "status"
  >
): Promise<string> => {
  try {
    const submissionsCollection = collection(
    ...
    if (
      now.seconds > assignment.dueDate.seconds &&
    ...
    await createNotification(schoolId, {
      ...
      title:
        status === "late"
          ? "Късно предаване на задача"
          : "Ново предаване на задача",
      message: `${submission.studentName} е предал работата си за "${
        assignment.title
      }"${status === "late" ? " (късно)" : ""}`,
...
export const gradeSubmission = async (
  ...
  try {
    ...
    await createNotification(schoolId, {
      ...
      message: `Вашето предаване за "${assignment.title}" е оценено.${
        feedback.grade ? ` Оценка: ${feedback.grade}` : ""
      }`,
      ...
      if (assignment.classIds && assignment.classIds.length > 0) {
        for (const classId of assignment.classIds) {
          ...
          if (classDoc.exists() && classDoc.data().studentIds) {
            classDoc
              .data()
              .studentIds.forEach((id: string) => studentIds.add(id));
          }
        }
      }
