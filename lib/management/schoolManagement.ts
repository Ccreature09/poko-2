  ...
  for (const user of users) {
    ...
    try {
      ...
      if (user.role === "student" && user.homeroomClassId) {
        const homeroomClassRef = await createOrGetHomeroomClass(
          schoolId,
...
export const createParentUser = async (
  ...
  parentData: Omit<
    Parent,
    "userId" | "role" | "childrenIds" | "inbox" | "password"
  >, // Exclude fields set automatically or managed elsewhere
...
export const loginUser = async (
  ...
  try {
    ...
    const userDoc = await getDoc(
      doc(db, "schools", selectedSchool, "users", userCredential.user.uid)
    );
    if (userDoc.exists()) {
...
export async function getStudentsInClass(
  ...
  if (students.length === 0) {
    console.log(
      `Trying secondary approach by document IDs for class ${classId}`
    );

    const idPromises: Promise<
      import("firebase/firestore").QuerySnapshot<
        import("firebase/firestore").DocumentData
      >
    >[] = [];
    for (let i = 0; i < studentIds.length; i += batchSize) {
      const idQuery = query(usersRef, where("role", "==", "student"));
