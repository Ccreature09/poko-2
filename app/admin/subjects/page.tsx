...
export default function SubjectManagement() {
  ...
  const handleTeacherSelectionChange = (teacherId: string, isSelected: boolean) => {
  ...
  const handleAddSubject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nameCheckQuery = query(subjectsRef, where("name", "==", subjectFormData.name));
  ...
  const handleEditSubject = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      const subjectRef = doc(db, "schools", user.schoolId, "subjects", selectedSubject.subjectId);
      ...
      for (const teacherId of teachersToRemove) {
        ...
        if (teacherDoc.exists()) {
          ...
          await updateDoc(teacherRef, {
            teachesSubjects: teachesSubjects.filter((subjectId: string) => subjectId !== selectedSubject.subjectId)
          });
        }
      }
  ...
  const handleDeleteSubject = async () => {
    ...
    try {
      ...
      classesSnapshot.forEach(docSnapshot => {
        ...
        if (classData.teacherSubjectPairs) {
          const isUsed = classData.teacherSubjectPairs.some(
            (pair: any) => pair.subjectId === selectedSubject.subjectId
          );
          if (isUsed) {
            isUsedInClass = true;
          }
      ...
      if (isUsedInClass) {
        toast({
          title: "Error",
          description: "Cannot delete subject because it is used in one or more classes",
          variant: "destructive",
        });
        setIsSubmitting(false);
        return;
      ...
      await deleteDoc(doc(db, "schools", user.schoolId, "subjects", selectedSubject.subjectId));
      ...
      <div className="flex-1 p-8 overflow-auto bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">Управление на предмети</h1>
              <p className="text-gray-600 mt-1">Създаване и управление на учебни предмети и присъединяване на учители</p>
            </div>
              ...
              <Dialog open={isAddSubjectDialogOpen} onOpenChange={setIsAddSubjectDialogOpen}>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Попълнете необходимата информация за създаване на нов учебен предмет
                    </DialogDescription>
                  </DialogHeader>
                            ...
                            <ScrollArea className="h-48">
                              <div className="space-y-2">
                                {teachers.map((teacher) => (
                                  <div key={teacher.userId} className="flex items-center space-x-2">
                                    <Checkbox 
                                      id={`teacher-${teacher.userId}`}
                                      checked={subjectFormData.teacherIds.includes(teacher.userId)}
                                      onCheckedChange={(checked) => handleTeacherSelectionChange(teacher.userId, checked === true)}
                                    />
                                    <Label htmlFor={`teacher-${teacher.userId}`} className="cursor-pointer">
              ...
              {isLoading ? (
                ...
                <>
                        ...
                        {filteredSubjects.length === 0 ? (
                          ...
                          filteredSubjects.map((subjectData, index) => (
                            <TableRow key={subjectData.subjectId}>
                              ...
                              <TableCell className="font-medium">{subjectData.name || 'Без име'}</TableCell>
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => handleDeleteClick(subjectData)}
                                  >
                  ...
                  <div className="mt-4 text-sm text-gray-500">
                    Показани {filteredSubjects.length} от {subjects.length} предмети
                  </div>
                </>
              )}
            </CardContent>
                        ...
                        {teachers.map((teacher) => (
                          <div key={teacher.userId} className="flex items-center space-x-2">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleTeacherSelectionChange(teacher.userId, checked === true)}
                            />
                            <Label htmlFor={`edit-teacher-${teacher.userId}`} className="cursor-pointer">
                              {teacher.firstName} {teacher.lastName}
                            </Label>
                          </div>
                        ))}
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Сигурни ли сте, че искате да изтриете този предмет? Това действие не може да бъде отменено.
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedSubject && (
            <div className="py-4">
              ...
              <div className="mt-4 text-sm">
                <p className="mt-1"><span className="font-medium">Учители:</span> {selectedSubject.teacherIds?.length || 0}</p>
              </div>
            </div>
          )}
