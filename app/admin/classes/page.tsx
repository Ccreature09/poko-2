...
import type { Teacher, Subject } from "@/lib/interfaces";
...
export default function ClassManagement() {
      ...
      classes.forEach(cls => {
        if (cls.teacherSubjectPairs && cls.teacherSubjectPairs.length > 0) {
          cls.teacherSubjectPairs.forEach(pair => {
            ...
            if (teacherId && subjectId && teacherToSubjects[teacherId] && !teacherToSubjects[teacherId].includes(subjectId)) {
              teacherToSubjects[teacherId].push(subjectId);
            }
            
            if (teacherId && subjectId && subjectToTeachers[subjectId] && !subjectToTeachers[subjectId].includes(teacherId)) {
  ...
  useEffect(() => {
    ...
    if (educationLevelFilter !== "all") {
      result = result.filter(cls => cls.educationLevel === educationLevelFilter);
    }
  ...
  const fetchSubjects = async () => {
    ...
    try {
      const subjectsRef = collection(doc(db, "schools", user.schoolId), "subjects");
  ...
  }, [classFormData.gradeLevel, classFormData.section, classFormData.namingType]);
  ...
  const handleTeacherSubjectPairChange = (index: number, field: string, value: string | boolean) => {
    const newPairs = [...classFormData.teacherSubjectPairs];
  ...
  const getFilteredTeachers = (pairIndex: number): TeacherData[] => {
    ...
    if (isEditClassDialogOpen && currentPair.teacherId) {
      const teacherIds = [...(teacherSubjectMappings.subjectToTeachers[currentPair.subjectId] || [])];
  ...
  const getFilteredSubjects = (pairIndex: number): SubjectData[] => {
    ...
    if (isEditClassDialogOpen && currentPair.subjectId) {
      const subjectIds = [...(teacherSubjectMappings.teacherToSubjects[currentPair.teacherId] || [])];
  ...
  const handleAddClass = async (e: React.FormEvent) => {
    ...
    if (classFormData.teacherSubjectPairs.some(pair => !pair.teacherId || !pair.subjectId)) {
      toast({
        title: "Error",
        description: "All teacher-subject pairs must have both teacher and subject selected",
        variant: "destructive",
      });
      return;
    }
    
    const homeroomTeachers = classFormData.teacherSubjectPairs.filter(pair => pair.isHomeroom);
    ...
    try {
      ...
      const nameCheckQuery = query(classesRef, where("name", "==", classFormData.name));
      ...
      for (const pair of classFormData.teacherSubjectPairs) {
        const teacherRef = doc(db, "schools", user.schoolId, "users", pair.teacherId);
  ...
  const handleEditClass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const previousTeacherIds = selectedClass.teacherSubjectPairs.map(pair => pair.teacherId);
      const currentTeacherIds = classFormData.teacherSubjectPairs.map(pair => pair.teacherId);
      
      const teachersToRemove = previousTeacherIds.filter(id => !currentTeacherIds.includes(id));
      const teachersToAdd = currentTeacherIds.filter(id => !previousTeacherIds.includes(id));
      ...
      for (const teacherId of teachersToRemove) {
        ...
        if (teacherDoc.exists()) {
          ...
          if (teachesClasses.includes(selectedClass.name)) {
            await updateDoc(teacherRef, {
              teachesClasses: teachesClasses.filter(className => className !== selectedClass.name)
            });
          }
        }
      }
  ...
  const handleEditClick = (classData: HomeroomClass) => {
    ...
    setClassFormData({
      ...
      name: classData.name || '',
      namingType: classData.name === `${classData.gradeLevel}${classData.section}` ? "standard" : "custom",
  ...
  const handleDeleteClass = async () => {
    ...
    setIsSubmitting(true);
    try {
      if (selectedClass.teacherSubjectPairs && selectedClass.teacherSubjectPairs.length > 0) {
        for (const pair of selectedClass.teacherSubjectPairs) {
          if (pair && pair.teacherId) {
            ...
            if (teacherDoc.exists()) {
              ...
              await updateDoc(teacherRef, {
                teachesClasses: teachesClasses.filter((className: string) => className !== selectedClass.name)
              });
            }
          }
        }
      ...
      await deleteDoc(doc(db, "schools", user.schoolId, "classes", selectedClass.classId));
      ...
      <div className="flex-1 p-8 overflow-auto bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">Управление на класове</h1>
              <p className="text-gray-600 mt-1">Създаване и управление на учебни класове и присъединяване на учители</p>
            </div>
              ...
              <Dialog open={isAddClassDialogOpen} onOpenChange={setIsAddClassDialogOpen}>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Попълнете необходимата информация за създаване на нов учебен клас
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <form onSubmit={handleAddClass} className="space-y-4 my-4">
                    <div className="space-y-2">
                      ...
                      <RadioGroup 
                        ...
                        onValueChange={(value) => setClassFormData({...classFormData, namingType: value as "standard" | "custom"})}
                        className="flex items-center space-x-4"
                      >
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="standard" id="naming-standard" />
                          <Label htmlFor="naming-standard">Стандартно (1А, 2Б, и т.н.)</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => setClassFormData({...classFormData, gradeLevel: parseInt(value)})}
                          >
                        ...
                        <div className="space-y-2">
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => setClassFormData({...classFormData, section: value})}
                          >
                            <SelectTrigger id="section">
                              <SelectValue placeholder="Изберете паралелка" />
                            </SelectTrigger>
                            <SelectContent>
                              {["А", "Б", "В", "Г", "Д", "Е", "Ж", "З", "И"].map((section) => (
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="education-level">Образователно ниво</Label>
                      <Select 
                        value={classFormData.educationLevel} 
                        onValueChange={(value) => setClassFormData({...classFormData, educationLevel: value as "primary" | "middle" | "high"})}
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="middle">Прогимназиален (5-7 клас)</SelectItem>
                          ...
                          <div key={index} className="border p-3 rounded-lg bg-gray-50">
                            <div className="grid grid-cols-2 gap-3 mb-2">
                              <div className="space-y-1">
                                <Label htmlFor={`teacher-${index}`}>Учител</Label>
                                <Select 
                                  value={pair.teacherId} 
                                  onValueChange={(value) => handleTeacherSubjectPairChange(index, "teacherId", value)}
                                >
                                  <SelectTrigger id={`teacher-${index}`}>
                                    <SelectValue placeholder="Изберете учител" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {(pair.subjectId ? getFilteredTeachers(index) : teachers).map((teacher) => (
                              ...
                              <div className="space-y-1">
                                ...
                                <Select 
                                  ...
                                  onValueChange={(value) => handleTeacherSubjectPairChange(index, "subjectId", value)}
                                >
                                  <SelectTrigger id={`subject-${index}`}>
                                    <SelectValue placeholder="Изберете предмет" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {(pair.teacherId ? getFilteredSubjects(index) : subjects).map((subject) => (
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-2">
                                <Checkbox 
                                  ...
                                  onCheckedChange={(checked) => handleTeacherSubjectPairChange(index, "isHomeroom", checked === true)}
                                />
                                <Label htmlFor={`homeroom-${index}`} className="cursor-pointer">
                                  Класен ръководител
                                </Label>
                              </div>
                ...
                <div className="md:w-56">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="middle">Прогимназиален етап</SelectItem>
              ...
              {isLoading ? (
                ...
                <>
                        ...
                        {filteredClasses.length === 0 ? (
                          ...
                          filteredClasses.map((classData, index) => (
                            <TableRow key={classData.classId}>
                              ...
                              <TableCell>
                                <Badge variant="outline" className={getEducationLevelBadgeStyle(classData.educationLevel)}>
                                  {classData.educationLevel === "primary" ? "Начален" : 
                                   classData.educationLevel === "middle" ? "Прогимназиален" : "Гимназиален"}
                                </Badge>
                              </TableCell>
                              <TableCell>
                                {classData.teacherSubjectPairs?.find(pair => pair.isHomeroom)?.teacherId ? (
                                  getTeacherName(classData.teacherSubjectPairs.find(pair => pair.isHomeroom)!.teacherId)
                                ) : (
                                  <span className="text-gray-400">Не е зададен</span>
                                )}
                              </TableCell>
                  ...
                  <div className="mt-4 text-sm text-gray-500">
                    Показани {filteredClasses.length} от {classes.length} класове
                  </div>
                </>
              )}
            </CardContent>
            ...
            <div className="space-y-2">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="edit-naming-standard">Стандартно (1А, 2Б, и т.н.)</Label>
                </div>
                <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <Checkbox 
                          ...
                          onCheckedChange={(checked) => handleTeacherSubjectPairChange(index, "isHomeroom", checked === true)}
                        />
                        <Label htmlFor={`edit-homeroom-${index}`} className="cursor-pointer">
                          Класен ръководител
                        </Label>
                      </div>
          ...
          {selectedClass && (
            <div className="py-4">
              <p className="font-medium">
                Клас: {selectedClass.name}
              </p>
              <p className="text-sm text-gray-500 mt-1">
                {selectedClass.educationLevel === "primary" ? "Начален етап" : 
                 selectedClass.educationLevel === "middle" ? "Прогимназиален етап" : "Гимназиален етап"}
              </p>
              
              <div className="mt-4 text-sm">
                <p className="mt-1"><span className="font-medium">Ученици:</span> {selectedClass.students?.length || 0}</p>
                <p className="mt-1"><span className="font-medium">Учебна година:</span> {selectedClass.academicYear}</p>
              </div>
              
              {selectedClass.students?.length > 0 && (
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-300 rounded-md text-yellow-700 text-sm">
                  <p className="font-medium">Внимание:</p>
                  <p>Този клас съдържа {selectedClass.students.length} ученици. При изтриване на класа, учениците ще останат в системата, но няма да бъдат асоциирани с никой клас.</p>
                </div>
              )}
            </div>
          )}
