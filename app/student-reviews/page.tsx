...
import type { Student, Subject, StudentReview, ReviewType } from "@/lib/interfaces";
...
export default function StudentReviews() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.schoolId || !selectedStudent) {
      toast({
        title: "Грешка",
    ...
    try {
      ...
      if (selectedSubject && selectedSubject !== "none" && subjectMap[selectedSubject]) {
      ...
      toast({
        title: "Успех",
        description: "Забележката беше добавена успешно.",
      });
      ...
      // Refresh reviews
      const updatedReviews = await getChildReviews(user.schoolId, selectedStudent);
      setStudentReviews(updatedReviews);
    } catch (error) {
      console.error("Error adding review:", error);
      toast({
        title: "Грешка",
        description: "Неуспешно добавяне на забележка.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="flex flex-col lg:flex-row h-screen">
      ...
      <div className="flex-1 p-8 overflow-auto bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold mb-8 text-gray-800">Забележки за ученици</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Изберете ученик, за да добавите или прегледате забележки
                </CardDescription>
              </CardHeader>
              <CardContent>
                        ...
                        {filteredStudents.map(student => (
                          <div
                            ...
                            className={`p-3 rounded-md cursor-pointer hover:bg-gray-100 transition-colors ${
                              selectedStudent === student.userId ? "bg-blue-50 border border-blue-200" : ""
                            }`}
                          >
                            <p className="font-medium">{student.firstName} {student.lastName}</p>
                            <p className="text-sm text-gray-500">Клас: {student.homeroomClassId}</p>
                          </div>
                        ))}
                        
                        {filteredStudents.length === 0 && (
                          <p className="text-center py-4 text-gray-500">Няма намерени ученици</p>
                        )}
                      </div>
                    </ScrollArea>
                  )}
            ...
            <Card>
              <CardHeader>
                <CardTitle>Добавяне на забележка</CardTitle>
                <CardDescription>
                  Създайте нова положителна или отрицателна забележка
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="review-type">Тип на забележката</Label>
                    <RadioGroup
                      value={reviewType}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="title">Заглавие на забележката</Label>
                    <Input
                      id="title"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="content">Съдържание на забележката</Label>
                    <Textarea
                      id="content"
              ...
              <CardFooter>
                <Button
                  ...
                  className={`w-full ${
                    reviewType === 'positive' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
                  }`}
                  disabled={!selectedStudent || isSubmitting}
                >
                  {isSubmitting ? "Изпращане..." : "Добави забележка"}
                </Button>
              </CardFooter>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Преглед на забележки</CardTitle>
                <CardDescription>
                  {selectedStudent 
                    ? `Всички забележки за избрания ученик` 
                    : `Изберете ученик, за да видите забележки`}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {!selectedStudent ? (
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <p className="text-gray-500">
                      Изберете ученик от списъка отляво, за да видите неговите забележки
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    <MessageSquare className="h-12 w-12 text-gray-300 mb-4" />
                    <p className="text-gray-500">
                      Няма намерени забележки за този ученик
                    </p>
                  </div>
                ) : (
                  <ScrollArea className="h-[400px] pr-4">
