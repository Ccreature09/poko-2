...
import { getChildQuizResults, getChildCheatingAttempts } from '@/lib/parentManagement';
import { 
  Dialog,
  DialogContent,
  DialogDescription,
...
const getStatusBadge = (quiz: Quiz, result?: QuizResult) => {
  if (!result) {
    ...
    if (isOverdue) {
      return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Пропуснат</Badge>;
    }
    
    if (quiz.availableFrom && new Date() < new Date(quiz.availableFrom.seconds * 1000)) {
      return <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200">Предстоящ</Badge>;
    }
    
    return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">Не е направен</Badge>;
  }
  ...
  if (result.completed) {
    ...
    if (percentScore >= 90) {
      return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Отличен ({percentScore}%)</Badge>;
    }
    if (percentScore >= 70) {
      return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Добър ({percentScore}%)</Badge>;
    }
    if (percentScore >= 50) {
      return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">Среден ({percentScore}%)</Badge>;
    }
    return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Под очакванията ({percentScore}%)</Badge>;
  }
  ...
  if (result.timestamp && !result.completed) {
    return <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200">Прекъснат</Badge>;
  }
  
  return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">В прогрес</Badge>;
};

// Format quiz availability time
const formatAvailabilityTime = (quiz: Quiz) => {
  if (quiz.availableFrom && quiz.availableTo) {
    return `${format(new Date(quiz.availableFrom.seconds * 1000), 'dd.MM.yyyy HH:mm')} - ${format(new Date(quiz.availableTo.seconds * 1000), 'dd.MM.yyyy HH:mm')}`;
  } else if (quiz.availableFrom) {
    return `От ${format(new Date(quiz.availableFrom.seconds * 1000), 'dd.MM.yyyy HH:mm')}`;
  } else if (quiz.availableTo) {
    return `До ${format(new Date(quiz.availableTo.seconds * 1000), 'dd.MM.yyyy HH:mm')}`;
  }
...
export default function ParentQuizzes() {
      ...
      try {
        ...
        for (const quiz of data.quizzes) {
          if (quiz.teacherId && !teacherNames[quiz.teacherId]) {
            try {
              const teacherDoc = await getDoc(doc(db, "schools", user.schoolId, "users", quiz.teacherId));
              if (teacherDoc.exists()) {
                const teacherData = teacherDoc.data();
                teacherNames[quiz.teacherId] = `${teacherData.firstName} ${teacherData.lastName}`;
              } else {
                teacherNames[quiz.teacherId] = "Неизвестен учител";
              }
            } catch (err) {
              console.error(`Error fetching teacher for quiz ${quiz.quizId}:`, err);
              teacherNames[quiz.teacherId] = "Неизвестен учител";
            }
          }
        }
  ...
  const getQuizzesByCategory = () => {
    ...
    const pending = quizzes.filter(quiz => {
      ...
      const isAvailable = 
        (!quiz.availableFrom || new Date(quiz.availableFrom.seconds * 1000) <= now) &&
  ...
  const calculateStats = () => {
    ...
    completedResults.forEach(result => {
      ...
      lowestScore = completedResults.length > 0 ? Math.min(lowestScore, percentage) : 0;
    });
    ...
    const quizzesWithCheatingAttempts = Object.keys(quizData.cheatingAttempts).length;
    ...
    <div className="flex flex-col md:flex-row min-h-screen">
      ...
      <div className="flex-1 p-4 md:p-8 overflow-auto bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold mb-2 text-gray-800">Тестове на детето</h1>
          <p className="text-gray-600 mb-6">Преглед на тестовете и резултатите на вашето дете</p>
          ...
          {!selectedChildId ? (
            ...
            <>
              ...
              <Card className="bg-white mb-6">
                <CardHeader>
                  ...
                  <CardDescription>Преглед на представянето на {selectedChild?.name} на тестове</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Quizzes summary */}
                    <div className="space-y-4">
                      <h3 className="text-sm font-medium text-gray-700">Тестове</h3>
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-green-50 p-3 rounded-md">
                          ...
                          <p className="text-2xl font-semibold text-green-700">{stats.completedQuizzes}</p>
                        </div>
                        <div className="bg-amber-50 p-3 rounded-md">
                          <p className="text-xs text-gray-500">Очакващи</p>
                          <p className="text-2xl font-semibold text-amber-700">{stats.pendingQuizzes}</p>
                        </div>
                        <div className="bg-red-50 p-3 rounded-md">
                          <p className="text-xs text-gray-500">Пропуснати</p>
                          <p className="text-2xl font-semibold text-red-700">{stats.missedQuizzes}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Performance summary */}
                    <div className="space-y-4">
                      <h3 className="text-sm font-medium text-gray-700">Резултати</h3>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <p className={`text-lg font-semibold ${getScoreColor(stats.averageScore)}`}>
                            {stats.averageScore}%
                          </p>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <p className={`text-lg font-semibold ${getScoreColor(stats.highestScore)}`}>
                            {stats.highestScore}%
                          </p>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <p className={`text-lg font-semibold ${getScoreColor(stats.lowestScore)}`}>
                            {stats.lowestScore}%
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <h3 className="text-sm font-medium text-gray-700">Допълнителна информация</h3>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <p className="text-sm text-gray-500 flex items-center">
              ...
              <Card className="bg-white">
                <CardHeader>
                  ...
                  <CardDescription>Преглед на всички тестове и резултати</CardDescription>
                </CardHeader>
                <CardContent>
                  <Tabs 
                    value={activeTab} 
                        ...
                        <TableBody>
                          {filteredQuizzes.length > 0 ? (
                            filteredQuizzes.map((quiz) => {
                              ...
                              const cheatingAttempts = getCheatingAttemptsForQuiz(quiz.quizId);
                                ...
                                <TableRow key={quiz.quizId}>
                                  <TableCell className="font-medium">
                                    ...
                                    {cheatingAttempts.length > 0 && (
                                      <Badge variant="outline" className="ml-2 bg-amber-50 text-amber-700 border-amber-200">
                                        <AlertTriangle className="h-3 w-3 mr-1" /> Предупреждения
                                      </Badge>
                                    )}
                                  </TableCell>
                                  <TableCell>
                                    {quizData?.teacherNames[quiz.teacherId] || "Учител"}
                                  </TableCell>
                                  <TableCell>
                                    {quiz.timeLimit ? `${quiz.timeLimit} минути` : 'Неограничено'}
                                  </TableCell>
                                  <TableCell>
                                  ...
                                  <TableCell>
                                    {result && result.completed ? (
                                      <span className={getScoreColor(Math.round((result.score / result.totalPoints) * 100))}>
                                  ...
                                  <TableCell>
                                    <Button 
                                      variant="ghost" 
                                      size="icon" 
                                      onClick={() => showQuizDetails(quiz, result)}
                                    >
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow">
              ...
              <h3 className="text-lg font-medium text-gray-800">Няма тестове за показване</h3>
              <p className="text-gray-500 mt-2">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center">
                      ...
                      <span className="text-gray-600">Време за изпълнение:</span>
                    </div>
                    <div>
                      {selectedQuiz.timeLimit ? `${selectedQuiz.timeLimit} минути` : 'Неограничено'}
                    </div>
                    ...
                    <div>
                      {formatAvailabilityTime(selectedQuiz)}
                    </div>
                    ...
                    <div>
                      {selectedQuiz.points || selectedQuiz.questions.reduce((sum, q) => sum + (q.points || 0), 0)}
                    </div>
                    ...
                    <div>
                      {quizData?.teacherNames[selectedQuiz.teacherId] || "Неизвестен учител"}
                    </div>
                  </div>
                </div>
                ...
                {selectedResult ? (
                  <div className="space-y-3">
                        ...
                        {selectedResult.completed && (
                          <>
                            <div className="flex justify-between items-center">
                              ...
                              <span className={getScoreColor(Math.round((selectedResult.score / selectedResult.totalPoints) * 100))}>
                                {selectedResult.score} / {selectedResult.totalPoints}
                              </span>
                            </div>
                            
                            <div className="flex justify-between items-center">
                              <span className="text-gray-600">Процент верни отговори:</span>
                              <span className={getScoreColor(Math.round((selectedResult.score / selectedResult.totalPoints) * 100))}>
                                {Math.round((selectedResult.score / selectedResult.totalPoints) * 100)}%
                              </span>
                            </div>
                            
                            <div className="space-y-1">
                              <div className="flex justify-between items-center">
                                <span className="text-gray-600">Напредък по въпроси:</span>
                                <span>
                                  {Object.keys(selectedResult.answers || {}).length} / {selectedQuiz.questions.length} въпроса
                                </span>
                              </div>
                              <Progress 
                                value={Math.round((Object.keys(selectedResult.answers || {}).length / selectedQuiz.questions.length) * 100)} 
                                className="h-2"
                              />
                            </div>
                            ...
                            <div className="flex justify-between items-center">
                              <span className="text-gray-600">Време за изпълнение:</span>
                              <span>{formatTimeSpent(selectedResult.totalTimeSpent)}</span>
                            </div>
                            
                            <div className="flex justify-between items-center">
                              <span className="text-gray-600">Дата на изпълнение:</span>
                              <span>{format(new Date(selectedResult.startedAt.seconds * 1000), 'dd.MM.yyyy HH:mm')}</span>
                            </div>
                          </>
                        )}
                      </div>
                    ...
                    {selectedResult.securityViolations && selectedResult.securityViolations > 0 && (
                      <div className="border-t pt-3">
                        ...
                        <p className="text-sm text-amber-600">
                          По време на изпълнението на теста са отбелязани {selectedResult.securityViolations} предупреждения 
