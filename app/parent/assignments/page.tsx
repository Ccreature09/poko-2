...
const getStatusBadge = (assignment: Assignment, submission?: AssignmentSubmission) => {
  ...
  const isOverdue = new Date() > new Date(assignment.dueDate.seconds * 1000) && !submission;
  
  if (submission) {
    if (submission.status === 'graded') {
      return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Оценено</Badge>;
    } else if (submission.status === 'submitted') {
      return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Предадено</Badge>;
    } else if (submission.status === 'late') {
      return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">Закъсняло</Badge>;
    } else if (submission.status === 'resubmitted') {
      return <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Предадено отново</Badge>;
    }
  }
  
  if (isOverdue) {
    return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Просрочено</Badge>;
  }
  
  return <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200">Изчакващо</Badge>;
};
...
const getGrade = (submission?: AssignmentSubmission) => {
  if (!submission || submission.status !== 'graded' || !submission.feedback?.grade) {
    return '-';
  }
...
export default function ParentAssignments() {
  ...
  const showAssignmentDetails = (assignment: Assignment, submission?: AssignmentSubmission) => {
  ...
  const filteredAssignments = assignmentsByCategory[activeTab];
    ...
    <div className="flex flex-col md:flex-row min-h-screen">
      ...
      <div className="flex-1 p-4 md:p-8 overflow-auto bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold mb-2 text-gray-800">Задачи на детето</h1>
          <p className="text-gray-600 mb-6">Преглед и проследяване на училищните задачи на вашето дете</p>
          ...
          {!selectedChildId ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <Card className="bg-white">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-500">Общо задачи</p>
                      <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                        <FileText className="h-5 w-5 text-blue-600" />
                      </div>
                    </div>
                    <h3 className="text-2xl font-bold mt-2">{assignmentData.assignments.length}</h3>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-white">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-500">Предадени</p>
                ...
                <Card className="bg-white">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-500">Чакащи</p>
                ...
                <Card className="bg-white">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-500">Просрочени</p>
              ...
              <Card className="bg-white">
                <CardHeader>
                  ...
                  <CardDescription>Преглед на текущи и минали задачи</CardDescription>
                </CardHeader>
                <CardContent>
                  <Tabs 
                    value={activeTab} 
                        ...
                        <TableBody>
                          {assignmentsByCategory.all.length > 0 ? (
                            assignmentsByCategory.all
                              .sort((a: Assignment, b: Assignment) => b.dueDate.seconds - a.dueDate.seconds)
                              .map((assignment: Assignment) => {
                                const submission = assignmentData.submissions[assignment.assignmentId];
                                return (
                                  <TableRow key={assignment.assignmentId}>
                                    ...
                                    <TableCell>{getStatusBadge(assignment, submission)}</TableCell>
                                    <TableCell>{getGrade(submission)}</TableCell>
                                    <TableCell>
                                      <Button 
                                        variant="ghost" 
                                        size="icon" 
                                        onClick={() => showAssignmentDetails(assignment, submission)}
                                      >
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow">
              ...
              <h3 className="text-lg font-medium text-gray-800">Няма задачи за показване</h3>
              <p className="text-gray-500 mt-2">
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="flex justify-between items-center pt-2">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    До: {format(new Date(selectedAssignment.dueDate.seconds * 1000), 'PPP')}
                  </span>
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t pt-3">
                  <h3 className="text-sm font-medium mb-2">Статус на предаване:</h3>
                  <div className="flex items-center gap-2">
                    {getStatusBadge(selectedAssignment, selectedSubmission || undefined)}
                    <span className="text-sm">
                      {selectedSubmission 
                        ? `Предадено на ${format(new Date(selectedSubmission.submittedAt.seconds * 1000), 'PPP')}`
                        : 'Не е предадено'}
                    </span>
                  </div>
                </div>
                ...
                {selectedSubmission && (
                  <>
                    <div>
                      <h3 className="text-sm font-medium mb-1">Съдържание на предаденото:</h3>
                    ...
                    {selectedSubmission.status === 'graded' && selectedSubmission.feedback && (
                      <div>
                        <h3 className="text-sm font-medium mb-1">Обратна връзка от учителя:</h3>
                        <div className="text-sm p-3 bg-blue-50 rounded-md whitespace-pre-wrap">
                          {selectedSubmission.feedback.comment || "Учителят не е оставил обратна връзка."}
                        </div>
                        
                        <div className="mt-2 flex items-center gap-2">
                          <span className="text-sm font-medium">Оценка:</span>
                          <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                            {selectedSubmission.feedback.grade?.toFixed(2) || 'Без оценка'}
                          </Badge>
                        </div>
                      </div>
                    )}
