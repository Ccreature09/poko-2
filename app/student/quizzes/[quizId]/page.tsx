...
import { AlertCircle, Timer, CheckCircle2, AlertTriangle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function QuizPage() {
    ...
    try {
      ...
      const result = {
        ...
        startedAt: quizStartTime
          ? Timestamp.fromDate(quizStartTime)
          : Timestamp.now(),
  ...
  const handleTimeUp = useCallback(async () => {
    ...
    toast({
      ...
      description:
        "Времето за теста изтече и той ще бъде предаден автоматично.",
      variant: "destructive",
    });
  ...
  const handleCheatingDetected = useCallback(
    async (type: CheatAttemptType, description: string) => {
      console.debug(
        `[QuizPage] Cheating detected - Type: ${type}, Description: ${description}`
      );
  ...
  const handleTabSwitch = useCallback(() => {
    ...
    setWarningCount((prev) => {
      ...
      if (quiz?.securityLevel === "extreme" && newCount >= 3) {
        console.debug(
          "[QuizPage] Excessive tab switches detected, auto-submitting"
        );
  ...
  const saveProgress = useCallback(async () => {
    ...
    saveProgressTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        await saveQuizProgress(quizId as string, answers, currentQuestionIndex);
      } catch (err) {
  ...
  const loadQuiz = useCallback(
    async (mounted: boolean) => {
      ...
      if (!user || !quizId || !mounted) {
        console.debug("[QuizPage] Cannot load quiz - missing data:", {
          user,
          quizId,
          mounted,
        });
      ...
      try {
        ...
        if (!mounted) {
          console.debug(
            "[QuizPage] Component unmounted during load, abandoning"
          );
          return;
        }
        ...
        if (foundQuiz.timeLimit && timeRemaining === null) {
          console.debug(
            `[QuizPage] Setting initial time limit: ${foundQuiz.timeLimit} minutes`
          );
          const startTime = new Date();
          const endTime = new Date(
            startTime.getTime() + foundQuiz.timeLimit * 60 * 1000
          );
        ...
        if (mounted) {
          toast({
            title: "Грешка",
            description:
              "Грешка при зареждане на теста. Моля, опитайте отново.",
            variant: "destructive",
          });
          router.push("/student/quizzes");
        }
  ...
  useEffect(() => {
    if (!quiz || !quiz.questions[currentQuestionIndex] || isInitializing)
      return;
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      ...
      if (quiz.securityLevel === "extreme") {
        ...
        quiz.securityLevel === "high" ||
        quiz.securityLevel === "medium"
      ) {
        handleTabSwitch();
      }
    };
    ...
    const handleFocus = () => {
      ...
      const timeSinceLastFocus =
        (now.getTime() - visibilityRef.lastFocused.getTime()) / 1000;
      ...
      if (timeSinceLastFocus > 10 && quiz.securityLevel !== "low") {
        recordCheatAttempt(quizId as string, {
          type: "window_blur",
          description: `User was away from the quiz for ${Math.round(
            timeSinceLastFocus
          )} seconds`,
        });
      }
    ...
    const handleCopy = (e: ClipboardEvent) => {
      if (quiz.securityLevel !== "low") {
        ...
        toast({
          title: "Внимание",
          description:
            "Копирането на съдържание по време на тест не е позволено",
          variant: "destructive",
        });
  ...
  if (remainingAttempts <= 0) {
    console.debug(
      `[QuizPage] Access denied - No remaining attempts: ${remainingAttempts}`
    );
    console.debug(
      `[QuizPage] TookTest array for this user:`,
      quiz.tookTest?.filter((id) => id === user?.userId)
    );
      ...
      <div className="flex h-screen">
        ...
        <div className="flex-1 p-8 flex flex-col justify-center items-center">
          ...
          <h1 className="text-2xl font-bold mb-2">
            Вече сте изпълнили този тест
          </h1>
          <p className="text-muted-foreground mb-6">
            Не можете да започнете теста отново
          </p>
    ...
    <div className="flex h-screen">
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            if (checked) {
                              handleAnswerChange(currentQuestion.questionId, [
                                ...currentAnswers,
                                idx.toString(),
                              ]);
                            } else {
      ...
      <Dialog open={showWarningDialog} onOpenChange={setShowWarningDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Превключването между табове или напускането на страницата с теста
              не е позволено и може да бъде отчетено като опит за измама.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              Това действие е записано. Множество нарушения може да доведат до
              автоматично предаване на теста или дисквалификация.
            </p>
            <p className="text-sm font-semibold mt-2">
              Предупреждение {warningCount} от{" "}
              {quiz.securityLevel === "extreme"
                ? "2"
                : quiz.securityLevel === "high"
                ? "3"
      ...
      <Dialog open={showConfirmSubmit} onOpenChange={setShowConfirmSubmit}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Сигурни ли сте, че искате да предадете теста? Това действие не
          ...
          <div className="py-4">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Отговорени въпроси: {Object.keys(answers).length} от{" "}
      ...
      <Dialog open={showTimeAlert} onOpenChange={setShowTimeAlert}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button onClick={() => setShowTimeAlert(false)}>Разбирам</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
