...
import { BookOpen, Clock, ArrowLeft, BookOpenText, FileText, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function CourseDetails() {
  ...
  const goToNextTopic = () => {
    if (!selectedSubchapter || currentTopicIndex >= selectedSubchapter.topics.length - 1) return;
  ...
  const selectTopicByIndex = (index: number) => {
    if (!selectedSubchapter || index < 0 || index >= selectedSubchapter.topics.length) return;
          ...
          <div className="border-b pb-4 px-4 lg:px-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground mt-1 whitespace-pre-wrap">{course.description}</p>
              </div>
              ...
              <div className="p-4 bg-muted/10">
                ...
                >
                  {course.chapters.map((chapter, chapterIndex) => (
                    <AccordionItem key={chapter.title} value={`chapter-${chapterIndex}`} className="border-b-0">
                      ...
                      <AccordionContent>
                        ...
                        >
                          {chapter.subchapters?.map((subchapter) => (
                            <AccordionItem key={subchapter.subchapterId} value={`subchapter-${subchapter.subchapterId}`} className="border-b-0">
                              <AccordionTrigger 
            ...
            <div className="flex-1 p-4 lg:p-8 overflow-auto">
                ...
                <div className="max-w-3xl">
                  <Card className="mb-6">
                    <CardHeader className="border-b bg-muted/10">
                      <Badge variant="outline" className="mb-2 w-fit">{selectedSubchapter.title}</Badge>
                    ...
                    <CardFooter className="flex justify-between pt-2 border-t">
                      <div className="text-sm text-muted-foreground">
                        Тема {currentTopicIndex + 1} of {selectedSubchapter.topics.length}
                      </div>
                      <div className="flex gap-2">
                        ...
                        <Button 
                          ...
                          disabled={currentTopicIndex >= selectedSubchapter.topics.length - 1}
                        >
                          ...
                          {(chapter.subchapters?.length ?? 0) > 0 && (
                            <div className="space-y-4">
                              <Accordion type="multiple" className="w-full">
                                {chapter.subchapters?.map((subchapter) => (
                                  <AccordionItem key={subchapter.subchapterId} value={`overview-${subchapter.subchapterId}`} className="border-l border-b-0 pl-4">
              ...
              {user && ((user.role === "teacher" && course.teacherId === user.userId) || user.role === "admin") ? (
                <div className="flex items-center space-x-2 mt-4">
                  <Button variant={"outline"} onClick={() => router.push(`/${user.role}/courses/${courseId}/edit`)}>
                     Редактирай курса
                  </Button>
                  
                  <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                    <DialogTrigger asChild>
                      <Button variant="destructive" className="text-white">Изтрий курса</Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          ...
                          Всички учебни материали ще бъдат премахнати за постоянно.
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
