...
import { BookOpen, Users, Calendar, ArrowRight, PlusCircle, Edit, Trash } from "lucide-react";
...
export default function TeacherCourses() {
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      await deleteDoc(doc(db, "schools", user.schoolId, "courses", courseToDelete));
  ...
  const teacherCourses = courses.filter(course => course.teacherId === user.userId);
                ...
                <Card key={course.courseId} className="h-full overflow-hidden hover:shadow-lg transition-all duration-300 border hover:border-primary/20">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/teacher/courses/${course.courseId}/edit`}>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        <span>
                          {course.classIds && course.classIds.length > 0 
                            ? `${course.classIds.length} клас${course.classIds.length > 1 ? 'а' : ''}`
                      ...
                      <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        <span>{course.chapters?.length || 0} глав{course.chapters?.length !== 1 ? 'и' : 'а'}</span>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardFooter className="pt-0">
                    <div className="w-full flex items-center gap-2">
                      <Link href={`/teacher/courses/${course.courseId}`} className="flex-1">
      ...
      <AlertDialog open={!!courseToDelete} onOpenChange={(open: boolean) => !open && setCourseToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Това действие не може да бъде отменено. Курсът и всичките му материали ще бъдат изтрити.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
