...
import { updateQuiz } from "@/lib/management/quizManagement";
import {
  Select,
  SelectContent,
  SelectItem,
      ...
      try {
        ...
        setAvailableFrom(
          quizData.availableFrom
            ? new Date(quizData.availableFrom.toDate())
                .toISOString()
                .slice(0, 16)
            : ""
        );
        setAvailableTo(
          quizData.availableTo
            ? new Date(quizData.availableTo.toDate()).toISOString().slice(0, 16)
            : ""
        );
    ...
    try {
      ...
      await updateQuiz(user.schoolId, quizId as string, {
        ...
        availableFrom: availableFrom
          ? Timestamp.fromDate(new Date(availableFrom))
          : undefined,
        availableTo: availableTo
          ? Timestamp.fromDate(new Date(availableTo))
          : undefined,
        randomizeQuestions,
        randomizeChoices,
        allowReview,
