...
export default function TeacherTimetable() {
  ...
  const getDetailsForPeriod = (day: string, period: number): { subject: string; classId: string; subjectId: string; className?: string }[] => {
                            ...
                            <td key={day} className="px-4 py-2 text-center">
                              {details.map((detail, index) => (
                                <div key={index} className="space-y-1">
                                  ...
                                  {detail.classId && detail.subject !== '-' && (
                                    <Link 
                                      href={`/teacher/attendance?classId=${detail.classId}&subjectId=${detail.subjectId}&date=${encodeURIComponent(getDateForDay(day).toISOString())}&period=${period}&tab=manual-entry`}
