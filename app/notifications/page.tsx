...
export default function NotificationsPage() {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const counts = await getNotificationCountsByCategory(schoolId, user.userId, true);
      ...
      const fetchedNotifications = await getUserNotifications(schoolId, user.userId, options);
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      const category = activeCategory !== 'all' ? activeCategory as NotificationCategory : undefined;
      await markAllNotificationsAsRead(schoolId, user.userId, category);
      
      // Update local state
      setNotifications(prev => 
        prev.map(item => {
          if (!item.read && (activeCategory === 'all' || item.category === activeCategory)) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Tabs 
        ...
        onValueChange={(value) => setActiveCategory(value as NotificationCategory | 'all')}
      >
        ...
        {(['assignments', 'quizzes', 'grades', 'attendance', 'feedback', 'messages', 'system'] as const).map(category => (
          <TabsContent key={category} value={category} className="mt-0">
            <Card className="p-6">
              {isLoading ? (
                ...
                <EmptyState category={category} showOnlyUnread={showOnlyUnread} />
              )}
            </Card>
          </TabsContent>
        ))}
