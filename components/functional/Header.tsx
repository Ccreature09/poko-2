...
export default function Header() {
  ...
  const [categoryCounts, setCategoryCounts] = useState<Record<NotificationCategory, number>>({
    assignments: 0,
    quizzes: 0,
    grades: 0,
    attendance: 0,
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    try {
      ...
      if (!notification.read) {
        await markNotificationAsRead(user.schoolId, user.userId, notification.id);
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      await markAllNotificationsAsRead(user.schoolId, user.userId, categoryParam);
              ...
              <div className="flex items-center space-x-4 justify-self-end">
                ...
                <Popover onOpenChange={setIsNotificationOpen}>
                  ...
                  <PopoverContent className="w-96 p-0" align="end">
                    ...
                    <Tabs defaultValue="all" onValueChange={(value) => setActiveCategory(value as NotificationCategory | 'all')}>
                      <div className="border-b px-2">
                        <TabsList className="w-full bg-transparent">
                          <TabsTrigger value="all" className="flex-1 relative data-[state=active]:text-blue-600">
                            Всички
                            {unreadCount > 0 && (
                              <Badge className="absolute -top-1 -right-1 px-1.5 py-0.5 bg-red-500 text-white text-xs min-w-[18px] h-[18px] flex items-center justify-center rounded-full">
                                {unreadCount > 9 ? '9+' : unreadCount}
                      ...
                      {Object.keys(categoryCounts).map((category) => (
                        <TabsContent key={category} value={category} className="m-0">
                          <NotificationList 
                            ...
                            onNotificationClick={handleNotificationClick} 
                            emptyMessage={`Няма уведомления в категория ${getCategoryTitle(category as NotificationCategory)}`}
                          />
                        </TabsContent>
                      ))}
                    </Tabs>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                      <Link href="/profile/notifications">Настройки за известия</Link>
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
...
function NotificationList({ notifications, onNotificationClick, emptyMessage = 'Няма уведомления' }: NotificationListProps) {
  return (
              ...
              <div className="flex gap-3">
                ...
                {notification.icon && (
                  <div 
                    ...
                    style={{ backgroundColor: notification.color ? `${notification.color}15` : '#f3f4f6' }}
                  >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center ml-2">
                      ...
                      {notification.priority === 'high' && (
                        <span className="w-2 h-2 bg-red-500 rounded-full mr-2" title="Висок приоритет" />
                      )}
                      {notification.priority === 'urgent' && (
                        <span className="w-2 h-2 bg-red-500 rounded-full mr-2 animate-pulse" title="Спешно" />
                      )}
                      ...
                      {notification.actions.map((action, idx) => (
                        <Button 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (action.url) {
                              onNotificationClick({ ...notification, link: action.url });
                            }
                          }}
                        >
...
function formatTimestamp(timestamp: { seconds: number, nanoseconds: number }): string {
